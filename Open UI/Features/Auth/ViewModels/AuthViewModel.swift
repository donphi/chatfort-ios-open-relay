...
final class AuthViewModel {
        ...
        if let active = serverConfigStore.activeServer {
            ...
            if KeychainService.shared.hasToken(forServer: active.url),
               let cachedUser = Self.loadCachedUser(forServer: active.url) {
                ...
                // ChatFort override: always go through serverConnection so the
                // auto-connect view kicks in and routes to nativeProxyLogin.
    ...
    func disconnect() {
        ...
        currentUser = nil
        phase = .serverConnection
        // ChatFort override: keep the default server URL so auto-connect works.
        serverURL = "https://chat.chatfort.ai"
        apiKey = ""
        stopTokenRefreshTimer()
        // ChatFort override: clear Authentik session cookies so the proxy
        // requires re-authentication on next connect.
    ...
    func signOut() async {
        ...
        // ChatFort override: clear Authentik session cookies from HTTPCookieStorage
        // so the proxy correctly requires re-authentication on next login.
        if let cookies = HTTPCookieStorage.shared.cookies {
            for cookie in cookies {
                HTTPCookieStorage.shared.deleteCookie(cookie)
            }
    ...
    private func captureFlowCookiesAndResume(session: URLSession, serverURL: String) async {
        ...
        // Clear login state (keep isNativeProxyLoggingIn true through OAuth chain)
        nativeProxyPassword = ""
        nativeProxyTOTP = ""
        nativeProxyNeedsMFA = false
    ...
    /// Programmatically completes the OAuth login flow using existing Authentik session cookies.
    /// After the native proxy login, the user has a valid Authentik session in HTTPCookieStorage.
    /// This method follows the OAuth redirect chain via URLSession (which shares those cookies)
    /// to obtain the OpenWebUI JWT token — skipping the auth method selection screen entirely.
    private func performAutoOAuthLogin(serverURL: String, provider: String) async {
        let loginPath = "\(serverURL)/oauth/\(provider)/login"
        guard let loginURL = URL(string: loginPath) else {
            logger.warning("Auto-OAuth: invalid URL \(loginPath)")
            isNativeProxyLoggingIn = false
            phase = .authMethodSelection
            return
        }

        logger.info("Auto-OAuth: following redirect chain from \(loginPath)")

        // URLSession with shared cookie storage — has the Authentik session cookies
        // from the Flow Executor login that just completed.
        let sessionConfig = URLSessionConfiguration.default
        sessionConfig.httpCookieAcceptPolicy = .always
        sessionConfig.httpCookieStorage = HTTPCookieStorage.shared
        sessionConfig.httpShouldSetCookies = true
        ...
        do {
            ...
            // After following all redirects, look for the OpenWebUI JWT token cookie
            if let serverHost = URL(string: serverURL)?.host,
               let cookies = HTTPCookieStorage.shared.cookies {
                let tokenCookie = cookies.first { cookie in
                    cookie.name == "token" &&
                    (cookie.domain.contains(serverHost) || serverHost.contains(cookie.domain))
                }
                if let token = tokenCookie?.value,
                   !token.isEmpty,
                   token.split(separator: ".").count == 3,
                   token.count >= 50 {
                    logger.info("Auto-OAuth: captured JWT token from cookie — completing login")
                    isConnecting = false
            ...
            logger.warning("Auto-OAuth: no token captured — falling back to manual login")
            isNativeProxyLoggingIn = false
            phase = .authMethodSelection
        } catch {
            logger.warning("Auto-OAuth failed: \(error.localizedDescription) — falling back")
            isNativeProxyLoggingIn = false
            phase = .authMethodSelection
        }
    }
    ...
    private func connectSkippingProxyCheck(normalizedURL: String, proxyAuthCookies: [String: String], userAgent: String) async {
        ...
        if !apiKey.isEmpty {
            ...
            // ChatFort override: auto-complete OAuth using Authentik session cookies.
            // The native proxy login already authenticated with Authentik, so the
            // OAuth redirect chain will auto-complete without user interaction.
            if let provider = backendConfig?.oauthProviders?.enabledProviders.first {
                logger.info("Auto-OAuth: detected provider '\(provider)', starting programmatic login")
                await performAutoOAuthLogin(serverURL: normalizedURL, provider: provider)
            } else {
                isNativeProxyLoggingIn = false
                phase = .authMethodSelection
            }
