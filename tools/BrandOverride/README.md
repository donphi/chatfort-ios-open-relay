...
Put your ChatFort icon in the `Assets/` folder:

```
tools/BrandOverride/Assets/AppIcon.icon           # Icon Composer bundle (required)
tools/BrandOverride/Assets/AppIcon-preview.png    # exported PNG for in-app display (optional — auto-generated by ictool on CI)
```

The `.icon` file is created in **Icon Composer** (Xcode → Open Developer Tool → Icon Composer).
It contains all appearances (light, dark, tinted) and Liquid Glass effects in a single bundle.

For the in-app preview PNG, export from Icon Composer: File → Export → save as
...
| `02b_auth_native_login_view.json` | Custom NativeProxyLoginView with themed UI and auto-connect |
...
| App icon | The icon on the home screen | `Open UI/AppIcon.icon` | — | Icon Composer bundle | Open Relay icon | Your icon | Yes (if you place `Assets/AppIcon.icon`) |
| In-app icon | Icon shown on About, Login, Onboarding screens | `AppIconImage.imageset/IMG_0816.png` | — | PNG image | Open Relay icon | Your icon | Yes (from `Assets/AppIcon-preview.png`) |
| Widget icon | Icon in widget gallery and widgets | `OpenUIWidgets/AppIcon.icon` | — | Icon Composer bundle | Open Relay icon | Your icon | Yes (same `.icon` bundle) |
...
Replace `Assets/AppIcon.icon` with your new Icon Composer bundle. All appearances
(light, dark, tinted) are configured inside the `.icon` file. If you also have an
`AppIcon-preview.png`, update it too (or delete it and let `ictool` regenerate it
on the next build).
...
### "Icon Composer bundle missing" warning

You have not placed your icon yet. Create your icon in Icon Composer and save it at:
```
tools/BrandOverride/Assets/AppIcon.icon
```
...
| `.icon` | Icon Composer bundle (layered app icon with Liquid Glass) | Icon Composer (Xcode → Open Developer Tool → Icon Composer) |
